    ...
    if(!document){
      User.addUser(newUser, (err, user)=>{
        if(err){
          ...
          var process = spawn('python3', ["./sendMail.py",
            newUser.username,
            // "  201601441",
            req.body.password,
          ]);
...
router.post('/sendotp', (req, res, next) => {
  // console.log(req);
  var spawn = require("child_process").spawn;
  var process = spawn('python3', ["./sendOTP.py",
    req.body.username,
    // "201601441",
    req.body.otp,
  ]);
...
router.post('/trainmodel',(req,res,next)=>{
  ...
  var spawn = require("child_process").spawn;
  var process = spawn('python3', ["./corpusAndTrainCombined.py",
    req.body.tags,
    req.body.batch,
    req.body.course,
    req.body.branchCode,
...
router.post('/checkanswers',(req,res,next)=> {
  ...
  var spawn = require("child_process").spawn;
  var process = spawn('python3', ["./checkAndMakePdfAndSend.py",
    req.body.examNo ,
    req.body.username ,
    req.body.batch ,
    req.body.course ,
