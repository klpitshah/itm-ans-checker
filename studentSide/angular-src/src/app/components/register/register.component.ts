...
export class RegisterComponent implements OnInit {
  ...
  ngOnInit() {
    this.myForm = this._fb.group({
      batch: ['',[Validators.required, Validators.minLength(2), Validators.maxLength(2)]],
      ...
      collegeCode : ['',[Validators.required, Validators.minLength(3), Validators.maxLength(3)]],
      streamCode : ['',[Validators.required, Validators.minLength(2), Validators.maxLength(2)]],
      branchCode : ['',[Validators.required, Validators.minLength(2), Validators.maxLength(2)]]
    })
  }
  ...
  onRegisterSubmit(){
    ...
    var i;
    var prefixInInteger = this.myForm.value.batch + this.myForm.value.collegeCode + this.myForm.value.streamCode + this.myForm.value.branchCode;
    for (i = start_id; i <= end_id; i++) {

      var nameHelper;
      if(i < 10){
        nameHelper = prefixInInteger + "00" + i.toString();
      }

      else if(i < 100){
        nameHelper = prefixInInteger + "0" + i.toString();
      }

      else{
        nameHelper = prefixInInteger + i.toString();
      }

      const user = {
        name: nameHelper,
        // email: (prefixInInteger + i).toString() + 'gtu@gmail.com',

        // for debugging
        email : "160950107071gtu@gmail.com",
