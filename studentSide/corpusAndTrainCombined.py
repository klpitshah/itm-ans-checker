...
# x = '[{"name":"sdfjsidfjsdoifjsdoijfoisdfj"}]'
...
examNo = int(sys.argv[5])
# program = str(sys.argv[4])
streamCode = str(sys.argv[4])
branchCode = str(sys.argv[5])
...
for url in keypoints:
    ...
    with io.open("scrapped_{}_{}_{}_{}.txt".format(course, batch,streamCode + branchCode, examNo), "a+", encoding="utf-8") as f:
        f.write(final)
...
wiki_scrapped = "scrapped_{}_{}_{}_{}.txt".format(course, batch, streamCode+branchCode, examNo)
...
f2.close()
import pickle
pickle.dump(model, open('ans_checker_{}_{}_{}_{}.sav'.format(course, batch,streamCode + branchCode, examNo), 'wb'))
print("o bc")
sys.stdout.flush()